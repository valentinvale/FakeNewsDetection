...
# 5. Train the SVM model (Support Vector Machine)
...
if has_labels:
    ...
    # 9. ROC Curve
    fpr, tpr, _ = roc_curve(y_test, svm.predict_proba(X_test_vectors)[:, 1])
    roc_auc = auc(fpr, tpr)
    ...
    precision, recall, _ = precision_recall_curve(y_test, svm.predict_proba(X_test_vectors)[:, 1])
