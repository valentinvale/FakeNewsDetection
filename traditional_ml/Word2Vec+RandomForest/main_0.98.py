...
y_val_pred = rf.predict(X_val_vectors)
...
y_test_pred = rf.predict(X_test_vectors)
...
if has_labels:
    ...
    # 9. ROC Curve
    fpr, tpr, _ = roc_curve(y_test, rf.predict_proba(X_test_vectors)[:, 1])
    roc_auc = auc(fpr, tpr)
    ...
    precision, recall, _ = precision_recall_curve(y_test, rf.predict_proba(X_test_vectors)[:, 1])
